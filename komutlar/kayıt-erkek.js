...
const kayıtlı = message.guild.roles.find(r => r.id === "763382187919671317"); // ERKEK ROLÜNÜN İDSİ
  
// işartetini kaldırın ve "54 55 59 60".ci satırdaki // işarterleri yine kaldırın.   
const ikinci = message.guild.roles.find(r => r.id === "763382188699156480");  //stg.addRole(ikinci) , stg.addRole(ucuncu) var 
const ucuncu = message.guild.roles.find(r => r.id === "763382223243968512");    
...
//------------------------------------KANALLAR-----------------------------------\\ STG
  ...
  const yedekleme = message.guild.channels.find(c => c.id === "763382316735004692"); // KAYITLARI SİZE ÖZEL GÖSTERİR.
  const dogrulandi = client.emojis.find(emoji => emoji.name === "arista_yesilonay"); // EMOJİ İSMİ (SADECE İSİM : <> FALAN DEĞİL SADECE İSİM ÖRN: evet)
